...
exports.getHiddenGems = exports.getTrendingMovies = exports.getTopRated = exports.getTrending = void 0;
...
const getTrending = async () => {
    ...
    const { data: cachedTrendingCheck, error: cachedTrendingCheckError } = await supabase_1.default
        .from('trending_cache')
        .select('cached_at')
        .gte('cached_at', oneDayAgo)
        .limit(1);
    if (cachedTrendingCheckError)
        console.error(cachedTrendingCheckError);
    if (cachedTrendingCheck && cachedTrendingCheck.length > 0) {
        const { data: cachedTrending, error: cachedTrendingError } = await supabase_1.default
            .from('trending_cache')
            .select('anilist_id')
            .order('rank', { ascending: true });
        if (cachedTrendingError)
            console.error(cachedTrendingError);
    ...
    try {
        ...
        const { error: insertTrendiingError } = await supabase_1.default
            .from('trending_cache').insert(toInsertTrendingCache);
        if (insertTrendiingError)
            console.error(insertTrendiingError);
        return results;
    }
    catch (err) {
...
const getTopRated = async () => {
    ...
    const { data: cachedTopRatedCheck, error: cachedTopRatedCheckError } = await supabase_1.default
        .from('top_rated_cache')
        .select('cached_at')
        .gte('cached_at', oneWeekAgo)
        .limit(1);
    if (cachedTopRatedCheckError)
        console.error(cachedTopRatedCheckError);
    if (cachedTopRatedCheck && cachedTopRatedCheck.length > 0) {
        const { data: cachedTopRated, error: cachedTopRatedError } = await supabase_1.default
            .from('top_rated_cache')
            .select('anilist_id')
            .order('rank', { ascending: true });
        if (cachedTopRatedError)
            console.error(cachedTopRatedError);
    ...
    try {
        ...
        const { error: insertTopRatedError } = await supabase_1.default
            .from('top_rated_cache').insert(toInsertTopRatedCache);
        if (insertTopRatedError)
            console.error(insertTopRatedError);
        return results;
    }
    catch (err) {
...
const getTrendingMovies = async () => {
    const oneWeekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
    //get one entry that was less than one week ago
    const { data: cachedTrendingMoviesCheck, error: cachedTrendingMoviesCheckError } = await supabase_1.default
        .from('trending_movies_cache')
        .select('cached_at')
        .gte('cached_at', oneWeekAgo)
        .limit(1);
    if (cachedTrendingMoviesCheckError)
        console.error(cachedTrendingMoviesCheckError);
    if (cachedTrendingMoviesCheck && cachedTrendingMoviesCheck.length > 0) {
        const { data: cachedTrendingMovies, error: cachedTrendingMoviesError } = await supabase_1.default
            .from('trending_movies_cache')
            .select('anilist_id')
            .order('rank', { ascending: true });
        if (cachedTrendingMoviesError)
            console.error(cachedTrendingMoviesError);
    ...
    try {
        ...
        const { error: cacheInsertMoviesError } = await supabase_1.default
            .from('trending_movies_cache').insert(toInsertTrendingMoviesCache);
        if (cacheInsertMoviesError)
            console.error(cacheInsertMoviesError);
        return results;
    }
    catch (err) {
...
const getHiddenGems = async () => {
    const oneWeekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
    //get one entry that was less than one week ago
    const { data: cachedHiddenGemsCheck, error: cachedHiddenGemsCheckError } = await supabase_1.default
        .from('hidden_gems_cache')
        .select('cached_at')
        .gte('cached_at', oneWeekAgo)
        .limit(1);
    if (cachedHiddenGemsCheckError)
        console.error(cachedHiddenGemsCheckError);
    if (cachedHiddenGemsCheck && cachedHiddenGemsCheck.length > 0) {
        const { data: cachedHiddenGems, error: cachedHiddenGemsError } = await supabase_1.default
            .from('hidden_gems_cache')
            .select('anilist_id')
            .order('rank', { ascending: true });
        if (cachedHiddenGemsError)
            console.error(cachedHiddenGemsError);
    ...
    try {
        ...
        const toInsertGenres = selectedResults.flatMap(anime => (anime.genres.map(genre => ({
            anilist_id: anime.anilistId,
            genre: genre
        }))));
        await supabase_1.default
        ...
        const { error: cacheInsertError } = await supabase_1.default
            .from('hidden_gems_cache').insert(toInsertHiddenGemsCache);
        if (cacheInsertError)
            console.error(cacheInsertError);
...
exports.getHiddenGems = getHiddenGems;
